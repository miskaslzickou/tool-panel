...
async function fetchCarData() {
      // Using fetch API
const response = await fetch(`https://api.carapi.dev/v1/plate-to-vin/${num_plate.value}?country=${country.value}&token=${import.meta.env.VITE_CAR_API_KEY}`, {
  method: 'GET',
  headers: {
    'Content-Type': 'application/json'
  }
...
const specs = await decodeVIN(data.vin, country.value);
